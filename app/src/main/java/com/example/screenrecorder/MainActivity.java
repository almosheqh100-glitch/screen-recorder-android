...
public class MainActivity extends Activity {
 ...
 public void onCreate(Bundle state){
  ...
  record=new Button(this);box.addView(record);record.setOnClickListener(v->{if(ScreenRecordService.recording)stopService(new Intent(this,ScreenRecordService.class));else begin();});
 ...
 private void capture(){requesting=true;startActivityForResult(getSystemService(MediaProjectionManager.class).createScreenCaptureIntent(),1);}
 public void onRequestPermissionsResult(int code,String[] permissions,int[] results){super.onRequestPermissionsResult(code,permissions,results);if(code==3)capture();else if(code==2 && results.length>0 && results[0]==PackageManager.PERMISSION_GRANTED)begin();else ScreenRecordService.message="لم تُمنح صلاحية الميكروفون. يمكنك التسجيل بدون صوت.";}
 protected void onActivityResult(int code,int result,Intent data){super.onActivityResult(code,result,data);requesting=false;
  if(code==1 && result==RESULT_OK && data!=null){ScreenRecordService.starting=true;Intent service=new Intent(this,ScreenRecordService.class).putExtra("resultCode",result).putExtra("data",data).putExtra("mic",mic.isChecked());try{startForegroundService(service);}catch(RuntimeException e){ScreenRecordService.starting=false;ScreenRecordService.message="تعذر بدء الخدمة: "+e.getMessage();}}
